...
from hapi.pipelines.patches.gender import Gender
from hapi.pipelines.patches.ipc_phase import IpcPhase
from hapi.pipelines.patches.ipc_type import IpcType
from hapi.pipelines.patches.locations import Locations
from hapi.pipelines.patches.org_type import OrgType
from hapi.pipelines.patches.population_group import PopulationGroup
from hapi.pipelines.patches.population_status import PopulationStatus
...
class Pipelines:
    def __init__(
        ...
        self.locations = Locations(
            configuration=configuration, use_live=use_live
        )
        countries = configuration["HAPI_countries"]
        libhxl_dataset = AdminLevel.get_libhxl_dataset().cache()
        self.admins = Admins(configuration, self.locations, libhxl_dataset)
